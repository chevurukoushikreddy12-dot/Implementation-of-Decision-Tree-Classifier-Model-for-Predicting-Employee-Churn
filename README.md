...
Import libraries (pandas, sklearn, matplotlib, etc.).
Load dataset (CSV) into a DataFrame.
Inspect data: head(), info(), isnull().sum(), target distribution.
Handle missing values (drop or impute) and encode categorical variables (LabelEncoder).
Select features (X) and target (y).
Split data into train and test sets (e.g., 80/20, set random_state for reproducibility).
Initialize DecisionTreeClassifier (criterion = "entropy" or "gini").
Train model with .fit(X_train, y_train).
Predict on X_test and optionally on new samples.
Evaluate using accuracy_score, confusion_matrix, classification_report.
Visualize the trained tree with plot_tree and save/show the plot.
(Optional) Tune hyperparameters or perform cross-validation for better performance.
